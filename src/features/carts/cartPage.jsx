...
export default function CartPage() {
  const { carts } = useSelector((state) => state.cartSlice); {/* locally select vako carts lai tanni*/}
  const { user } = useSelector((state) => state.userSlice);
  const totalAmount = carts.reduce((a, b) => a + b.price * b.qty, 0); {/* product nikalni */}
  const dispatch = useDispatch();


  const [addOrder, { isLoading }] = useAddOrderMutation(); {/* order place garnalai */}
  const handleOrder = async () => {
    try {
      ...
      dispatch(clearCart()); // order vai sakey paxi cart lai khali garnu parxa
